...
## Analyst Automation — GX Series

These tickets automate the manual copy-paste collection workflow. No social media APIs used.

**Execution order (sequential):**

```
GX-1 (Extension Ingest API + API key)
  └─ GX-2 (Chrome Extension — one-click Send to Gito)
       └─ GX-4 (Extension auto-collect — scheduled background scraping)
GX-3 (Bulk URL import in submit page) — independent of GX-2, depends only on GX-1
```

- **GX-1** — Backend only. New `/api/items/extension-ingest` endpoint with API key auth + API key display in Sources page.
- **GX-2** — Chrome extension. Floating "→ Gito" button on text selection; context menu item; platform-aware DOM extraction on X, Threads, Reddit, Instagram.
- **GX-3** — Submit page "Bulk import" tab. Paste up to 30 URLs, server fetches metadata (Reddit via `.json`, others via `og:` tags), checklist review, batch submit.
- **GX-4** — Extends GX-2 extension. Alarm-based scheduler opens search pages in background tabs, extracts posts, auto-POSTs to Gito. Replaces manual search browsing entirely.
