...
export async function POST(req: Request) {
  ...
  // Insert OP tweet item
  const op = tweets.find((t) => t.isOriginalPost) ?? tweets[0];
  const threadItem: NewIngestedItem = {
    platform: "twitter",
    externalId: threadUrl,
    url: threadUrl,
    title,
    body: threadBody ?? op.body ?? null,
    author: author ?? op.author ?? null,
  ...
  const replyItems: NewIngestedItem[] = replies.map((t, i) => ({
    ...
    body: t.body,
    author: t.author,
    publishedAt: safeDate(t.timestamp),
    entityId: resolvedEntityId,
    subtype: "x_reply",
    rawJson: null,
  }));
  ...
  // Fetch all inserted reply IDs (handles pre-existing dedup)
  const replyExternalIds = replyItems.map((r) => r.externalId!).filter(Boolean);
  const replyRows = replyExternalIds.length > 0
    ? await db
        .select({ id: ingestedItems.id })
        .from(ingestedItems)
