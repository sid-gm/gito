...
type QueueEntry =
  | { type: "keyword"; term: string; platforms?: Array<"twitter" | "threads"> }
...
async function processKeywordEntry(
  ...
  state: ActiveRunState,
  account: Account,
  platformsOverride?: Array<"twitter" | "threads">,
...
async function startCollectRun(
  ...
  // Stored configs may still contain legacy platforms (e.g. "reddit") — drop them.
  const keywordPlatforms = config.platforms.filter(
    (p): p is "twitter" | "threads" => p === "twitter" || p === "threads",
  );

  const entries: QueueEntry[] = [];
  if (keywordPlatforms.length > 0) {
    for (const term of config.terms) {
      entries.push({ type: "keyword", term, platforms: keywordPlatforms });
    }
  }
